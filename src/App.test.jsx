...
describe('revised hero', () => {
  ...
  it('reports when the clipboard fallback fails', async () => {
    Object.defineProperty(document, 'execCommand', { configurable: true, value: vi.fn().mockReturnValue(false) });
    Object.defineProperty(window, 'isSecureContext', { configurable: true, value: false });
    render(<App />);

    fireEvent.click(screen.getAllByRole('button', { name: 'Copy CA' })[0]);

    await waitFor(() => expect(screen.getByText('Could not copy the contract address.')).toBeTruthy());
  });
});
